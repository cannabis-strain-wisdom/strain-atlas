(() => {
  ...
  const sourceDeclaredCannabinoidSummary = cultivar => {
    const declarations = Array.isArray(cultivar?.cannabinoids?.presentation?.sourceDeclarations) ? cultivar.cannabinoids.presentation.sourceDeclarations : [];
    if (!declarations.length) return null;
    const numeric = declarations.filter(item => Number.isFinite(item?.value) && item?.unit === '%');
    if (!numeric.length) return null;
    const values = numeric.map(item => item.value);
    const scopes = new Set(numeric.map(item => item.evidenceScopeRef).filter(Boolean));
    const sampleScoped = numeric.every(item => item.evidenceScope === 'sample');
    const unverified = numeric.every(item => item.analysisVerified === false);
    return {
      count: numeric.length,
      scopeCount: scopes.size || numeric.length,
      min: Math.min(...values),
      max: Math.max(...values),
      sampleScoped,
      unverified
  ...
  const decorateSourceDeclaredCannabinoids = (root, cultivar) => {
    ...
    const section = root.querySelector('.ucd-cannabinoids');
    if (!section || section.querySelector('[data-cannabinoid-source-context="v1"]')) return false;
    const heading = section.querySelector('h2, h3, .ucd-section-title');
    const context = document.createElement('div');
    context.className = 'ucd-cannabinoid-source-context';
    context.dataset.cannabinoidSourceContext = 'v1';
    const label = document.createElement('strong');
    label.textContent = summary.sampleScoped ? '公式掲載の検体データ' : '公式掲載データ';
    const range = document.createElement('span');
    range.textContent = summary.min === summary.max ? `${summary.min}%` : `${summary.min}〜${summary.max}%`;
    const note = document.createElement('p');
    const countText = summary.sampleScoped ? `${summary.scopeCount}検体について公開された数値です。` : `${summary.count}件の公式掲載値です。`;
    ...
    context.append(label, range, note);
    if (heading?.parentNode === section) heading.insertAdjacentElement('afterend', context); else section.prepend(context);
  ...
  const unavailableTerpeneReason = cultivar => {
    ...
    if (aromaSupported) return { title: '個別テルペンは確認できていません', text: '香りの情報は確認されていますが、その香りからテルペン成分を推測することはしていません。この品種に直接結びつく個別テルペンの分析値・成分一覧を、現在の採用資料では確認できないため掲載していません。' };
    return { title: 'テルペン情報は確認できていません', text: 'この品種に直接結びつく個別テルペンの分析値・成分一覧を、現在の採用資料では確認できないため掲載していません。確認できない成分を他の品種や香りの情報から推測して補うことはしていません。' };
  };
  const addUnavailableTerpene = (root, cultivar) => {
    ...
    const profile = root.querySelector('.ucd-profile'); const nav = profile?.querySelector('.ucd-profile-nav'); const panels = profile?.querySelector('.ucd-profile-panels');
    if (!profile || !nav || !panels) return false;
    const id = `ucd-${cultivar.id}-terpene`; const button = document.createElement('button');
  ...
  const govern = async () => {
    ...
    const catalog = await loadCatalog(); const cultivar = catalog?.cultivars?.find(item => item?.id === root.dataset.publicDetailId);
  ...
  const style = document.createElement('style'); style.id = 'public-presentation-contract-v1-style';
  style.textContent = `
    ...
    .ucd-cannabinoid-source-context { margin:8px 14px 4px;padding:10px 12px;border:1px solid rgba(216,189,98,.18);border-radius:12px;background:rgba(216,189,98,.035); }
    .ucd-cannabinoid-source-context strong { display:inline-block;color:#d8bd62;font-size:10px;letter-spacing:.04em; }
    .ucd-cannabinoid-source-context span { display:inline-block;margin-left:8px;color:#eef3ef;font-size:14px;font-weight:800; }
    .ucd-cannabinoid-source-context p { margin:5px 0 0;color:#93a098;font-size:9px;line-height:1.6; }
