...
if (shamanMorphology.horizontalOverflow) throw new Error('Shaman Morphology detail has horizontal overflow');
  ...
  if (flavorTarget) {
    ...
    flavor = await evalv(`(()=>{const root=document.querySelector('.detail-public-v1[data-public-detail-id="${flavorTarget.id}"]');const panel=root?.querySelector('[data-profile-kind="flavor"]');return {id:${JSON.stringify(flavorTarget.id)},text:panel?.innerText||'',items:[...panel?.querySelectorAll('.ucd-flavor-terms span')||[]].map(n=>({raw:(n.dataset.flavorPublicRaw||n.querySelector('strong')?.textContent||n.textContent||'').trim(),text:(n.textContent||'').trim()})),horizontalOverflow:root.scrollWidth>root.clientWidth+1}})()`);
