...
if (appleSensory.overflow) throw new Error('Apple Fritter sensory presentation has horizontal overflow');
  ...
  const doSiIdentityState = await evalv("(()=>{const keys=[...document.querySelectorAll('[data-ft-node-key]')].map(n=>n.dataset.ftNodeKey);const names=[...document.querySelectorAll('.csw-ft-node strong')].map(n=>n.textContent.trim());const lineage=document.querySelector('#detail-shell .ucd-lineage > div');const entry=lineage?.querySelector('[data-family-tree-entry=\"v1\"]');const evidence=lineage?.querySelector(':scope > .ucd-evidence-row');return {keys,uniqueKeys:new Set(keys).size,aliasNode:names.includes('GSC'),unpublishedClickable:document.querySelectorAll('.csw-ft-node.is-unpublished[data-ft-node-id]').length,entryBeforeEvidence:!!entry&&!!evidence&&entry.parentElement===body&&evidence.parentElement===body&&body.lastElementChild===evidence&&Boolean(entry.compareDocumentPosition(evidence)&Node.DOCUMENT_POSITION_FOLLOWING)}})()");
