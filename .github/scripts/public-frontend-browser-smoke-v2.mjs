...
async function main() {
  ...
  const sampleScopePrepared = await evalv(`(async()=>{const catalog=await window.__CSWRuntimeCatalogPromise;const cultivar=catalog?.cultivars?.find(x=>x.id==='rush-of-siam');const item=cultivar?.cannabinoids?.presentation?.items?.[0];const root=document.querySelector('.detail-public-v1[data-public-detail-id="rush-of-siam"],.ucd-root[data-public-detail-id="rush-of-siam"]');if(!item||!root)return false;window.__CSWSampleScopeSmokeBackup={evidenceType:item.evidenceType,contextLabel:item.contextLabel,evidenceScope:item.evidenceScope};item.evidenceType='SAMPLE_MEASUREMENT';item.contextLabel='確認検体1件';item.evidenceScope='sample';root.appendChild(document.createComment('sample-scope-smoke'));return true})()`);
  if (!sampleScopePrepared) throw new Error('Cannabinoid sample-scope synthetic fixture could not be prepared');
  await waitFor(() => evalv(`(()=>{const root=document.querySelector('.detail-public-v1[data-public-detail-id="rush-of-siam"],.ucd-root[data-public-detail-id="rush-of-siam"]');const cell=root?.querySelector('.ucd-cannabinoid-grid')?.children?.[0];const label=cell?.querySelector('[data-csw-cannabinoid-sample-scope="v1"]');const value=cell?.querySelector('strong');return !!label&&label.textContent.trim()==='確認検体1件'&&value?.nextElementSibling===label&&root?.dataset.cswCannabinoidSampleScope==='v1'})()`), 'visible cannabinoid sample scope in collapsed summary');
  const sampleScopeState = await evalv(`(()=>{const root=document.querySelector('.detail-public-v1[data-public-detail-id="rush-of-siam"],.ucd-root[data-public-detail-id="rush-of-siam"]');const cells=[...root?.querySelectorAll('.ucd-cannabinoid-grid > div')||[]];return {labels:cells.map(cell=>cell.querySelector('[data-csw-cannabinoid-sample-scope="v1"]')?.textContent?.trim()||''),contract:window.__CSWCannabinoidSampleScopeV1||null}})()`);
  if (sampleScopeState.labels[0] !== '確認検体1件') throw new Error(`Cannabinoid sample scope label missing from summary: ${JSON.stringify(sampleScopeState)}`);
  if (sampleScopeState.labels.slice(1).some(Boolean)) throw new Error(`Cannabinoid sample scope leaked to non-sample values: ${JSON.stringify(sampleScopeState)}`);
  if (sampleScopeState.contract?.status !== 'PASS') throw new Error(`Cannabinoid sample scope decorator did not PASS: ${JSON.stringify(sampleScopeState)}`);
  await evalv(`(async()=>{const catalog=await window.__CSWRuntimeCatalogPromise;const cultivar=catalog?.cultivars?.find(x=>x.id==='rush-of-siam');const item=cultivar?.cannabinoids?.presentation?.items?.[0];const backup=window.__CSWSampleScopeSmokeBackup||{};if(item){for(const key of ['evidenceType','contextLabel','evidenceScope']){if(backup[key]===undefined)delete item[key];else item[key]=backup[key]}}delete window.__CSWSampleScopeSmokeBackup;const root=document.querySelector('.detail-public-v1[data-public-detail-id="rush-of-siam"],.ucd-root[data-public-detail-id="rush-of-siam"]');root?.appendChild(document.createComment('sample-scope-smoke-reset'));return true})()`);
...
if (appleSensory.overflow) throw new Error('Apple Fritter sensory presentation has horizontal overflow');
  ...
  await waitFor(() => evalv("(()=>{const labels=[...document.querySelectorAll('.csw-ft-edge-label')].map(n=>n.textContent);return labels.includes('選抜クローン')&&labels.includes('BX')})()"), 'Do-Si-Dos Family Tree edge labels');

  const familyTreeDoSiDos = await evalv("(()=>({state:window.__CSWFamilyTreeViewV1||null,center:document.querySelector('.csw-ft-node.is-current .csw-ft-copy strong')?.textContent||'',currentMeta:document.querySelector('.csw-ft-node.is-current .csw-ft-copy small')?.textContent||'',cswMetaCount:[...document.querySelectorAll('.csw-ft-node:not(.is-current) .csw-ft-copy small')].filter(n=>n.textContent==='CSW').length,footer:document.querySelector('.csw-ft-footer')?.textContent||'',ogkb:!!document.querySelector('[data-ft-node-id=\"ogkb\"]'),faceOffBx1:!!document.querySelector('[data-ft-node-id=\"face-off-og-bx1\"]'),gsc:!!document.querySelector('[data-ft-node-id=\"girl-scout-cookies\"]'),faceOffRoot:!!document.querySelector('[data-ft-node-id=\"face-off-og\"]'),faceOffUnpublished:[...document.querySelectorAll('.csw-ft-node.is-unpublished strong')].some(n=>n.textContent==='Face Off OG'),labels:[...document.querySelectorAll('.csw-ft-edge-label')].map(n=>n.textContent),levelLabels:[...document.querySelectorAll('.csw-ft-level')].map(n=>n.dataset.levelLabel||''),arrowedPathCount:[...document.querySelectorAll('.csw-ft-svg path')].filter(n=>n.getAttribute('marker-end')==='url(#csw-ft-arrow-v1)').length,depths:[...document.querySelectorAll('.csw-ft-level')].map(n=>Number(n.dataset.depth)),pageOverflow:document.documentElement.scrollWidth>window.innerWidth+1}))()");
  ...
  if (!familyTreeDoSiDos.labels.includes('選抜クローン') || !familyTreeDoSiDos.labels.includes('BX') || familyTreeDoSiDos.labels.includes('親')) throw new Error('Family Tree special-only relation labels invalid: ' + JSON.stringify(familyTreeDoSiDos.labels));
  for (const requiredLevelLabel of ['祖先（2世代前）','親','現在']) {
    if (!familyTreeDoSiDos.levelLabels.includes(requiredLevelLabel)) throw new Error('Family Tree generation level label missing: ' + requiredLevelLabel);
  }
  if (familyTreeDoSiDos.arrowedPathCount !== familyTreeDoSiDos.state?.edgeCount) throw new Error('Family Tree directional arrows incomplete: ' + JSON.stringify(familyTreeDoSiDos));
  ...
  for (const testCase of typedFamilyTreeCases) {
    ...
    await waitFor(() => evalv("window.__CSWFamilyTreeViewV1?.relationTypes?.includes('" + testCase.type + "') && [...document.querySelectorAll('.csw-ft-edge-label')].some(n=>n.textContent==='" + testCase.label + "')"), testCase.id + ' typed Family Tree labels ready');
